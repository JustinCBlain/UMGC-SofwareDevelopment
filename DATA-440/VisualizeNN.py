...
class Layer():
    ...
    def __line_between_two_neurons(self, neuron1, neuron2, weight=0.4,
        ...
        if abs_weight > 0.5:
            ...
            while ((not textoverlaphandler.getspace(
                    [txt_x_pos - 0.5, txt_y_pos - 0.5, txt_x_pos + 0.5,
    ...
    def draw(self, layerType=0, weights=None, textoverlaphandler=None):
        ...
        for neuron in self.neurons:
            ...
            if self.previous_layer:
                for previous_layer_neuron in self.previous_layer.neurons:
                    self.__line_between_two_neurons(neuron,
        ...
        if layerType == 0:
            ...
            pyplot.text(x_text, self.y, 'Hidden Layer ' + str(layerType),
                        fontsize=12)
...
class TextOverlappingHandler():
    ...
    def getspace(self, test_coordinates):
        ...
        if self.cells[x_left_pos, y_botttom_pos] and self.cells[
            x_left_pos, y_top_pos] \
                and self.cells[x_right_pos, y_top_pos] and self.cells[
            x_right_pos, y_botttom_pos]:
    ...
    def draw(self, weights_list=None):
        ...
        for i in range(len(self.layers)):
            ...
            if i == 0:
                ...
                layer.draw(layerType=-1, weights=weights_list[i - 1],
                           textoverlaphandler=overlaphandler)
            else:
                layer.draw(layerType=i, weights=weights_list[i - 1],
                           textoverlaphandler=overlaphandler)
